...
We see can instruction ahead **call sub_4010FD** with the string WNcry@2ol7, which is passed to the function **call sub_401DAB**. The function is quite complex, but to my general knowledge, it is loading a Resource module called XIA. It compares if one of the loaded modules is called c.wncry in a while loop. 
